...
module.exports = function createBuilder(rootPath) {
  ...
  /**
   * Truncates the root path from the given path
   * and returns the resulting path.
   * 
   * @param  {String} path
   * @return {String}
  ...
  function truncate(path) {
    ...
    // normalize the path
    var normalizedPath = NODE_PATH.normalize(path);

    // check if the path is within the required root
    if (!isPathWithin(normalizedPath)) {
      
      throw new IllegalPath(path);

    } else {
      return '/' + NODE_PATH.relative(rootPath, normalizedPath);
    }
  }
  ...
  rootPathBuilder.truncate  = truncate;
